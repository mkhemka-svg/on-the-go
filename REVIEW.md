...
**12.1** [PASS] `expo-notifications` and `expo-device` installed. `constants/notifications.ts` handles permission registration, Android channel setup, and four notification triggers: collaborator invited (`invite.tsx`), itinerary item added (`itinerary.tsx`), voting reminder after tutorial 2, and trip-start-day notification scheduled from `choose-destination.tsx`. Listeners registered in `_layout.tsx`.
